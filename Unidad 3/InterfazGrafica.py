...
if __name__ == "__main__":
    ...
    ventana.show()
    sys.exit(app.exec())
#| Elemento del código                                 Principio POO que aplica        | Para qué se usa                                    
# `class App(QWidget)`                                Herencia                    Hereda funciones de ventana de `QWidget`.            
# `self.guardar`, `self.consultar`                    Composición                  Usa otras clases dentro de la interfaz.              
# Métodos `accion_...`                                Encapsulación y Abstracción   Manejan acciones sin exponer detalles internos.      
# Campos y botones (`QLineEdit`, `QPushButton`, etc.) Objetos**                     Representan componentes individuales de la interfaz. 
